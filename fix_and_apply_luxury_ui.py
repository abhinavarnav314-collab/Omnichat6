...
if 'handleCopyCode' not in msgl_c:
    ...
    """
    msgl_c = re.sub(r'\{displayMessages\.length === 0 && \(.*?<\/div>\n\s*\)\}', lambda m: empty_state.strip(), msgl_c, flags=re.DOTALL)
...
@layer utilities {
  ...
  .prose blockquote {
    @apply border-l-4 border-blue-500 bg-blue-50 dark:bg-blue-900/20 py-1 px-4 rounded-r-lg italic shadow-sm;
  }
  .prose table {
    @apply w-full border-collapse my-4 text-sm rounded-lg overflow-hidden shadow-sm border border-slate-200 dark:border-slate-700;
  }
  .prose th {
    @apply bg-slate-100 dark:bg-slate-800 p-3 text-left font-semibold border-b dark:border-slate-700;
  }
  .prose td {
    @apply p-3 border-b dark:border-slate-700;
  }
  .prose a {
    @apply text-blue-600 dark:text-blue-400 hover:underline hover:text-blue-700 dark:hover:text-blue-300 font-medium;
  }
}
"""
    write_file(css_path, css_c)

# 9. ApiKeyManager Provider Health Dashboard and polish
apimanager_path = 'src/components/Settings/ApiKeyManager.tsx'
apim_c = read_file(apimanager_path)
if 'Activity' not in apim_c:
    apim_c = apim_c.replace("import { Eye, EyeOff, Save, Trash } from 'lucide-react';", "import { Eye, EyeOff, Save, Trash, Activity, CheckCircle2, AlertCircle } from 'lucide-react';")
    
    # Add state for health
    apim_c = apim_c.replace("const [show, setShow] = useState<Record<string, boolean>>({});", "const [show, setShow] = useState<Record<string, boolean>>({});\n  const [health, setHealth] = useState<Record<string, 'checking' | 'ok' | 'error'>>({});")
  ...
  const handleTest = async (id: string, keyVal: string) => {
    ...
    """
    apim_c = apim_c.replace("const handleSave = async", test_func.strip() + "\n  const handleSave = async")
    
    # Enhance the custom provider rendering
    custom_repl = """
                  <button onClick={() => setShow(s => ({...s, [p.id]: !s[p.id]}))} className="p-2 border rounded dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700">
                    {show[p.id] ? <EyeOff size={16} /> : <Eye size={16} />}
                  </button>
                  <button onClick={() => handleTest(p.id, key)} title="Test Connection" className="p-2 border rounded dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 text-blue-500">
    ...
    """
    apim_c = re.sub(r'<button onClick=\{.*?EyeOff.*?<\/button>\s*<button onClick=\{.*?handleSave.*?<\/button>', lambda m: custom_repl.strip(), apim_c, count=1, flags=re.DOTALL)
    
    # Enhance standard providers
    std_repl = """
              <button onClick={() => setShow(s => ({...s, [p.id]: !s[p.id]}))} className="p-2 border rounded dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700">
                {show[p.id] ? <EyeOff size={16} /> : <Eye size={16} />}
              </button>
              <button onClick={() => handleTest(p.id, keys[p.id] || '')} title="Test Connection" className="p-2 border rounded dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 text-blue-500">
                {health[p.id] === 'checking' ? <Activity size={16} className="animate-spin" /> : health[p.id] === 'ok' ? <CheckCircle2 size={16} className="text-green-500" /> : health[p.id] === 'error' ? <AlertCircle size={16} className="text-red-500" /> : <Activity size={16} />}
              </button>
              <button onClick={() => handleSave(p.id)} className="p-2 bg-[var(--accent-color)] text-white rounded hover:bg-[var(--accent-color)] shadow-sm">
                <Save size={16} />
    ...
    """
    apim_c = re.sub(r'<button onClick=\{.*?EyeOff.*?<\/button>\s*<button onClick=\{.*?handleSave.*?<\/button>', lambda m: std_repl.strip(), apim_c, flags=re.DOTALL)
    
    write_file(apimanager_path, apim_c)

print("Luxury UI and features correctly implemented")
